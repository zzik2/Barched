    ...
    public HitResult raycastHitResult(float f, Entity entity) {
        ...
        if (attackRange != null) {
            ...
            if (hitResult instanceof BlockHitResult) {
                hitResult = GameRendererAccessor.invokeFilterHitResult(hitResult, entity.getEyePosition(f), d);
            }
        }

        if (hitResult == null || hitResult.getType() == HitResult.Type.MISS) {
            double e = this.entityInteractionRange();
            hitResult = ((GameRendererAccessor) Minecraft.getInstance().gameRenderer).invokePick(entity, d, e, f);
        }
